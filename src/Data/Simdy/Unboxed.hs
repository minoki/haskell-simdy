...
mapX :: forall m a b. (SIMD m, VU.Unbox a, UnboxSIMD m a, VU.Unbox b, UnboxSIMD m b) => (forall f. SIMD f => f a -> f b) -> VU.Vector a -> VU.Vector b
  ...
  let goVec !i = if i + m <= n
                 then
                   do let !s = unsafeIndexUnboxedSIMD @m v i
                      unsafeWriteUnboxedSIMD result i (f s)
                      goVec (i + m)
                 else
                   goScalar i
  ...
  let goVec !i = if i + m <= n
                 then
                   do let !s0 = unsafeIndexUnboxedSIMD @m v0 i
                      ...
                      unsafeWriteUnboxedSIMD result i (f s0 s1)
                      goVec (i + m)
                 else
                   goScalar i
