# simdy -- A Haskell library for SIMD programming
...
These types are isomorphic to fixed-length lists, but with more efficient representations. For example, `X4 a` is isomorphic to `(a, a, a, a)`.
...
instance (SIMD f, SIMDBits) => MiniBits (f a) -- pseudocode
```

## Supported compilers and architectures

This library requires a modern GHC (at least 9.6). SIMD functionality is only available on certain architectures. However, you can always use non-SIMD implementation of the data types by enabling the `no-simd` package flag.
...
Some code is generated by a script. Run `cabal run script/Gen.hs` after cloning the repository.
