...
They are instances of the `SIMD` type class, along with `Identity` (a 1-element vector):
...
Element types such as `Float` and `Word16` are instances of the `SIMDElement` class. Basic operations are provided: `broadcast` (similar to `replicate n x`), `liftSIMD` and `liftSIMD2` (lifting a unary or binary function over the lanes; similar to `map` and `zipWith`), and `selectSIMD` (choosing lanes from two vectors according to a mask; similar to `zipWith3 (\c x y -> if c then x else y)`).
...
This library requires a modern GHC (9.8.3 at minimum). Some later point releases are also excluded because of compiler bugs; see the comment at the top of `simdy.cabal` for the list and the corresponding GHC issues. SIMD functionality is only available on certain architectures. However, you can always use a non-SIMD implementation of the data types by enabling the `no-simd` package flag.

Supported architectures are:

* x86_64 with
    * LLVM backend with GHC 9.8.3 or newer
...
* `haswell` (default: false)
    * Enable AVX2, FMA and F16C (F16C is not utilized yet).
...
These options apply only while the library itself is compiled. Since simdy's code is inlined into user code, programs using 256-bit or 512-bit vectors also need to be compiled with the corresponding options: `-mavx2 -mfma` for the `haswell` flag, and additionally `-mavx512f` for the `avx512` flag. Otherwise compilation of the user's code fails, because GHC rejects wide vector types that are not supported by the target.

## For developers

Some files are generated by scripts. Run scripts after changing them:

```
cabal run script/Gen.hs      # vector type modules (X2--X64) and HalfVector
cabal run script/GenVL.hs    # backend modules (Data.Simdy.Internal.VL128 etc.)
cabal run script/GenPrim.hs  # primop wrappers (Prim.hs)
```
